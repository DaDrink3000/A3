...
from services import *                           # BallotEvidentService, etc.
from routes import *                             # any shared route utilities
from routes.user_routes import user_bp           # blueprints
from routes.voting_routes import voting_bp
from routes.tally_routes import bp as tally_bp   # R07 tally routes
from security.mfa import bp as mfa_bp            # R01 MFA routes
...
# Health check (keep this single definition)
@app.get("/healthz")
def healthz():
    return "ok"
...
# --- R16: Geographic access controls (single, correct version) ---
@app.before_request
def geo_guard():
    """
    Apply geographic access controls to public endpoints (ballot casting, voter lookup).
    Allow internal endpoints (/healthz, /login, /dashboard, /tally) to pass.
    """
    path = request.path or ""
    protected = path.startswith("/ballot") or path.startswith("/voter") or path.startswith("/public-key")
    if protected:
        from middleware import enforce_geo   # relative import (works in container path /app)
        res = enforce_geo()
        if res is not None:
            return res
...
app.register_blueprint(tally_bp)   # R07 tally routes
app.register_blueprint(mfa_bp)     # R01 MFA routes

# --- Register R03 token endpoints (fix to relative import) ---
try:
...
if __name__ == "__main__":
    # Bind to all addresses so Docker can expose port 8001
